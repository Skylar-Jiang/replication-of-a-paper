...
class TimeAwareSelfAttention(torch.nn.Module):
    def __init__(self,hidden_unit,dropout_rate,head_num,dev):
        ...
        self.dev=dev
    def forward(self,queries,keys,values,abs_pos_K,abs_pos_V,time_mask,future_time_mask):
        Q=self.Q_w(queries)
        K =self.K_w(keys)
        V=self.V_w(values)
        ...
        output+=attention_weight.matmul(multi_abs_timeV)
        #为什么要reshape？？
...
class TiSASRec(torch.nn.Module):
    ...
    #对数据进行处理 全部变成emb
    def seq2feature(self,user_id,log_seqs):
        ...
        return final_output
    def forward(self,user_id,log_seqs,pos_seq,neg_seq):
        final_output = self.seq2feature(user_id,log_seqs)
    ...
    def predict(self,user_id,log_seqs,all_item_num):
        log_feats = self.seq2feature(user_id, log_seqs)
