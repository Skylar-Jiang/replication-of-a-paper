...
def main():
    ...
    sampler = data_sampler(user_train, usernum, itemnum, relation_matrix, batch_size=args.batch_size, maxlen=args.maxlen, n_workers=3)
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        for step in range(num_batch): # tqdm(range(num_batch), total=num_batch, ncols=70, leave=False, unit='b'):
            ...
            for param in model.abs_time_K_emb.parameters(): loss += args.l2_emb * torch.norm(param)
            for param in model.abs_time_V_emb.parameters(): loss += args.l2_emb * torch.norm(param)
